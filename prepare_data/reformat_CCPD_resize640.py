...
def annotation_from_name(file_name, img_folder):



    img = cv2.imread(os.path.join(img_folder, file_name))
    img_h, img_w, _ = img.shape
    resize_scale_x = 640 / img_w
    resize_scale_y = 640 / img_h
    ...
    x1y1[0] *= resize_scale_x
    x1y1[1] *= resize_scale_y
    x2y2[0] *= resize_scale_x
    x2y2[1] *= resize_scale_y
    x3y3[0] *= resize_scale_x
    x3y3[1] *= resize_scale_y
    x4y4[0] *= resize_scale_x
    x4y4[1] *= resize_scale_y
    ...
    # cv2.circle(img, (x1, y1), 1, (0, 255, 0), 10)
    # cv2.circle(img, (x2, y2), 1, (0, 255, 0), 10)
    # cv2.circle(img, (x1y1[0], x1y1[1]), 1, (0, 0, 255), 5)
    # cv2.circle(img, (x2y2[0], x2y2[1]), 1, (0, 0, 255), 5)
    # cv2.circle(img, (x3y3[0], x3y3[1]), 1, (0, 0, 255), 5)
    # cv2.circle(img, (x4y4[0], x4y4[1]), 1, (0, 0, 255), 5)


    # print((x1, y1, x2, y2, int(x1y1[0]), int(x1y1[1]), int(x2y2[0]), int(x2y2[1]), int(x3y3[0]), int(x3y3[1]), int(x4y4[0]), int(x4y4[1])))
...
def generate_data_list(ccpd_path='/home/hylink/eclipse-workspace/CCPD2019/'):
    ...
    image_roots = [
                   ...
                   # ccpd_path + 'ccpd_weather',
                    'E:/plate_recognition/CCPD_custom/img_pos',
    ]
    ...
    for root in image_roots:
        ...
        for file_name in file_name_list_train:
            location_annotation = annotation_from_name(file_name, root)
...
def generate_data_list_neg(ccpd_path='/home/hylink/eclipse-workspace/CCPD2019/'):

    image_roots = [
                   # ccpd_path + 'ccpd_np',
                    'E:/plate_recognition/CCPD_custom/img_neg',
    ]
    ...
    if not os.path.exists(os.path.dirname(train_list_file_path)):
        os.makedirs(os.path.dirname(train_list_file_path))
    fout_train = open(train_list_file_path, 'a') # w
    fout_test = open(test_list_file_path, 'a') # a
...
if __name__ == '__main__':
    ...
    # generate_data_list(ccpd_path='E:/plate_recognition/CCPD2019/')
    generate_data_list_neg(ccpd_path='E:/plate_recognition/CCPD2019/')
    # show_image()
    # dataset_statistics()
